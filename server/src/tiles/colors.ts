import { CompoundOf, NBT_String } from "./nbt";
...
export function getColor(block: { Name: NBT_String, Properties: CompoundOf<Record<string, NBT_String>> }) {
  if (block.Name.value in colorMapping) {
    return colors[colorMapping[block.Name.value]];
  }

  let match = Object.values(colors).findIndex(v => {
    let matches = v.match.test(block.Name.value);
    v.match.lastIndex = 0;
    if (v.exclude) {
      matches = matches && !v.exclude.test(block.Name.value)
      v.exclude.lastIndex = 0;
    }
    return matches;
  });

  colorMapping[block.Name.value] = match === -1 ? "NONE" : Object.keys(colors)[match];

  return match === -1 ? undefined : Object.values(colors)[match];
}
